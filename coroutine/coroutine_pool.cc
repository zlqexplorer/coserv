...
#include "coserv/coroutine/coroutine_pool.h"
...
CoroutinePool* GetCoroutinePool() {
  if (!t_coroutine_container_ptr) {
    t_coroutine_container_ptr = new CoroutinePool(gConfig->m_cor_pool_size, gConfig->m_cor_stack_size);
  }
...
Coroutine::ptr CoroutinePool::getCoroutineInstanse() {
  ...
  if (s == (void*)-1) {
    LOG_ERROR << "Start TinyRPC failed, faild to mmap get coroutine memory size\n";
    return nullptr;
  }
  char* s1 = s;
